...
public class RulesOf6005 {
    ...
    public static boolean mayUseCodeInAssignment(boolean writtenByYourself,
        ...
        // If you wrote it yourself (or with teammates in a team project), always allowed
        if (writtenByYourself) {
            return true;
        }

        // Otherwise, it must not be coursework from 6.005 (past or present)
        if (writtenAsCourseWork) {
            return false;
        }

        // It must be available to all students (e.g., library, StackOverflow)
        ...
        // If the assignment requires implementation, you cannot copy external code
        if (implementationRequired) {
            return false;
        }

        // Otherwise, external source code may be used
        return true;
    }
